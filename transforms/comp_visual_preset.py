...
def visual_preset(ctx: Context) -> None:
    ...
    for vpreset in vmf.by_class["comp_visual_preset"]:
        ...
        if conv_bool(vpreset["tm_enable"]):
            tm_name = vpreset["tonemapper"]
        ...
        if conv_bool(vpreset["fog_enable"]):
            fog_ent = vpreset["fog_controller"]
        ...
        if conv_bool(vpreset["cc_enable"]):
            filename = vpreset["cc_filename", ""]
        ...
        if conv_bool(vpreset["dof_enable"]):
            dof_controller = vpreset["dof_controller"]
