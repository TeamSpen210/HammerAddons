...
from srctools.vmf import conv_kv, VMF
...
def numeric_transition(ctx: Context) -> None:
    ...
    for ent in ctx.vmf.by_class['comp_numeric_transition']:
        apply_numeric_transition(ctx.vmf, ent)


def apply_numeric_transition(vmf: VMF, ent: Entity) -> None:
    """Modify a single transition ent."""
    ...
    if io_type not in ('io', 'kv'):
        ...
        try:
            targ_ent = next(vmf.search(target))
    ...
    if transform_type == 'speed':
        ...
        result = (
            ascii_lowercase[max(0, min(25, round(compute_point(x) * 25)))]
            for x in points
        )
    else:
